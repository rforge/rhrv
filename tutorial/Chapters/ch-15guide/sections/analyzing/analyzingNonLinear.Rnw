...
\textit{sampleEntropy} field
as expected: The \textit{calculations} field  stores $h_q(m,r)$
whereas that the \textit{statistic} stores the estimate of the
sample entropy. 
...
\textit{NonLinearAnalysis[[index]]\$lyapunov\$computations}
list. The maximum
Lyapunov exponent is stored under the 
\textit{NonLinearAnalysis[[index]]\$lyapunov\$statistic}.
<<accessingData>>=
...
\textit{NonLinearAnalysis[[index]]\$dfa\$computations}.
The fluctuation function and the windows used for the computations can be 
obtained using\textit{\$fluctuation.function} and 
\textit{\$window.sizes} on
\textit{NonLinearAnalysis[[index]]\$dfa\$computations}, respectively. 
