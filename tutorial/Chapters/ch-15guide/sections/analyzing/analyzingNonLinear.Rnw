...
in order to show the expected behaviour of a trully nonlinear RR time series and
avoid some issues when dealing with a noisy process. An example with a trully
...
widely-used nonlinear analysis algorithms. Some of the algorithms will not
be presented here, but the will be presented in section 
\ref{seq:nonlinearAdvanced}.
...
hrv.data = SurrogateTest(hrv.data, significance = 0.05,
    useFunction = timeReversibility, tau=4)
@
...
test is low. We have use it here for illustrative purposes.
...
estimating
an appropiate time lag based on the study of the autocorrelation function of a 
given time series:
\begin{itemize}
...
value specified by the user (\textit{first.value} method and value parameter.
\end{itemize}
...
print(kTimeLag)
@
Most of the nonlinear algorihtms in \textit{RHRV} accept as input both time lag 
and embedding
...
considering that it is close to the limit value 1 (\textit{treshold. Default
0.95}); the maximum relative change in $E1(d)$ with respect to $E1(d-1)$ in 
order to consider that the $E1$ function has been stabilized and it will stop
changing (\textit{maximumRelativeValue}. Default = 0.01)and the \textit{doPlot} 
parameter (If TRUE a plot of $E1(d)$ and $E2(d)$ is shown).
...
and the \textit{maxEmbeddingDim} parameters (i.e., the correlation sum is 
computedfor the embeddings ranging from \textit{minEmbeddingDim} to 
\textit{maxEmbeddingDim}). The time lag needed for reconstruct the phase space
is provided by the \textit{timeLag}
...
points used to compute this range can be specified with 
the \textit{pointsRadius} function.
The Theiler window is specified using the \textit{theilerWindow} parameter. 
Finally, a log-log plot of the correlation sum Vs. the radius by setting
\textit{doPlot = TRUE} (default).\\
...
Thus we shall use  \textit{timeLag = kTimeLag} and we may select a 
sequence of dimensions sorrounding
...
@
In order to access the correlation sum calculated with the 
\textit{CalculateCorrDim} function we may write:
<<accessingCorrDim1>>=
corr.struct = hrv.data$NonLinearAnalysis[[my.index]]$correlation
corrSum = corr.struct$corrSum
@

As usual, we may use the \textit{\$} operator from the R language to access
for access to the \textit{corrSum} attributes:
<<accessingCorrDim2>>=
...
A graphical representation of the correlation sum may also be obtained by using
...
\textit{EstimateCorrDim} function may be used. This function estimates the 
...
legend(x=1,y=-3, lty = rep(1,3), col = 1:3,
...
Since the correlation dimension is the most important of the fractal dimensions,
the generalized dimensions (including the information dimension) shall be 
presented in section \ref{seq:nonlinearAdvanced}.
...
must be high enough (in order to ``simulate" the limit $m \rightarrow \infty$).

Let's compute the sample entropy of our example file. First at all, we shall 
compute correlations sums using a high embedding dimension. In this example, 
we shall use at least four times the optimum embedding dimension
...
The results of the sample entropy estimations are stored under the sampleEntropy
as follows: The \textit{sampleEntropyCalculations} field  stores $h_q(m,r)$
whereas that the \textit{sampleEntropyEstimate} stores the estimates of the
sample entropy. 
The attributes of the \textit{sampleEntropyCalculations} field can be accessed 
as usual using \textit{\$}. \textit{sampleEntropyCalculations\$sample.entropy} 
...
on the radius. \textit{sampleEntropyCalculations\$radius} and 
\textit{sampleEntropyCalculations\$embedding.dims} return the radius and the 
embedding dimensions used for calculations:

<<accessingData>>=
se = hrv.data$NonLinearAnalysis[[my.index]]$
                        sampleEntropy$sampleEntropyCalculations
...
estimation = 
hrv.data$NonLinearAnalysis[[my.index]]$sampleEntropy$sampleEntropyEstimate
...
calculate the maxymum Lyapunov exponent should be computing the $S(t)$ 
...
there exist a minimum number of closeneighbours. 500 points are usually enough 
...
are working with the RR time series, the radius' range $[1-10]$ ms 
...
of the \textit{CalculagteMaxLyapunov} routine or using the
...
function we should check of for some temporal range it shows a linear 
behaviour. It that's the caser, its slope is an estimate of the maximal
Lyapunov exponent per unit of time. The \textit{EstimateMaxLyapunov} routine
allows the user to get always an
...
\textit{NonLinearAnalysis[[index]]\$lyapunov\$divergence}
list. The maximum
Lyapunov exponent is stored under the 
\textit{NonLinearAnalysis[[index]]\$lyapunov\$maxLyapunovExponent}.
<<accessingData>>=
lyapunov.results  = hrv.data$NonLinearAnalysis[[my.index]]$lyapunov
divergence.structure = lyapunov.results$divergence
max.exponent = lyapunov.results$maxLyapunovExponent
...
denote it by $t$: number of beats), we obtain the so-called Fluctuation 
function $F(t)$.
...
fluctuations) and $\alpha_2$ (
  the exponent for great values of $t$, characterizing long-term fluctuations). 
\end{itemize}
...
obtained by performing the regression in the $4\leq t \leq 16$ range wheras 
...
inputs the range of values for the windows size that will be used to estimate 
...
function. Note that this RR series only has one linear region.
...
\textit{NonLinearAnalysis[[index]]\$dfa\$dfaComputations}.
The fluctuation function and the windows used for the computations can be 
obtained using\textit{dfa.object\$fluctuation.function} and 
\textit{dfa.object\$window.sizes}, respectively. All the estimated exponents
are stored under the \textit{NonLinearAnalysis[[index]]\$dfa\$scalingExponents}
list.
<<obtainDFA>>=
#obtaining the object pf class "dfa"
dfa.object = hrv.data$NonLinearAnalysis[[1]]$dfa$dfaComputations
windows = dfa.object$window.sizes
fluctuation.f = dfa.object$fluctuation.function
# get the exponent 
scaling.exp = hrv.data$NonLinearAnalysis[[1]]$dfa$scalingExponents[[1]]
print(scaling.exp)
@
...
\textit{lmin} (minimal length of a diagonal line to be considered 
in the RQA. Default \textit{lmin} = 2), \textit{vmin} (minimal length of a 
vertical
line to be considered in the RQA. Default \textit{vmin = 2}), 
\textit{distanceToBorder} 
(In order to avoid border effects, the \textit{distanceToBorder} points near 
the border 
of the recurrence matrix are ignored when computing the RQA parameters. 
Default, 
\textit{distanceToBorder =2})  

An useful rule of thumb for selecting the \textit{radius} parameter is choosing
the radius so that the recurrence matrix is sparse although it must contain 
certain
vertical or diagonal lines (deterministic structures). 
...
@
Figure \ref{fig:recplot} shows the resulting recurrence plot. Please note that
this function does not follow the \textit{CalculateX},
\textit{PlotX} and \textit{EstimateX} convention. The recurrence plot could 
also be obtained with the \textit{recurrencePlot} function.
   
\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\textwidth]{figures/tutorial-RQA}
