...
avoid some issues when dealing with a noisy process. An example with a truly
...
heterogeneity of the nonlinear algorithms, we will explain how to access the 
computations for each algorithm as they are presented.
...
computed for the embeddings ranging from \textit{minEmbeddingDim} to 
...
behaviour. It that's the case, its slope is an estimate of the maximal
