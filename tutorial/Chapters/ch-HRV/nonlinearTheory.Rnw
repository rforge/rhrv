...
A wide variety of nonlinear statistics can be calculated using \textit{RHRV}, 
including largest Lyapunov exponent (\textit{CalculateMaxLyapunov}, 
\textit{EstimateMaxLyapunov} and \textit{PlotMaxLyapunov}),
generalized correlation dimension (\textit{CalculateCorrDim}, 
\textit{EstimateCorrDim} 
and \textit{PlotCorrDim}), SD1/SD2 of Poincar\'{e} plots 
(\textit{PoincarePlot}),
detrended fluctuation analysis (\textit{CalculateDFA}, \textit{EstimateDFA} and 
\textit{PlotDFA}),
sample entropy (\textit{CalculateSampleEntropy}, \textit{EstimateSampleEntropy} 
and \textit{PlotSampleEntropy}) and recurrence quantification analysis 
(\textit{RQA}). Table \ref{tab:nonlinearStats} summarizes the most important 
...
leaving some details for later.\\
...
large enough). Thus, the user should plot $log_{10}(C(r))$ Vs. 
$log_{10}(r)$ for several
embedding dimensions when looking for the correlation dimension and, if for 
some range $log_{10}(C(r))$ shows a similar linear behaviour in different 
...
performing a linear regression of $log10(Cq(r))\;Vs.\;(q-1)log10(r)$.
...
dimension when setting the order $q = 1$. In order to compute the information
dimension in practical applications, it is possible to demonstrate that the 
...
the the average radius that contains a given portion (a "fixed-mass") of the 
total points in the phase space. By performing
a linear regression of $\log(p)\;Vs.\;\log(<r>)$ (being $p$ the fixed-mass of 
the total points), an estimate of $D_1$ is obtained. 
...
calculated as the standard deviation along the line of identity. In this way
