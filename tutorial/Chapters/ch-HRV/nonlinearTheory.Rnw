...
sections, we shall present a quick theorical review of all these methods,
...
in this package. see sections  \ref{seq:infDimTheory} and \ref{seq:infDim}). 
The considerations discussed for the correlation dimension estimate are also 
valid for these generalized dimensions.
\subsubsection{Information dimension\ref{seq:infDimTheory}}%TODO
Cooming soon!!
