...
licenses (e.g., aHRV). Even although others are free, they require the purchase of 
...
Suunto and WFDB.
\item RHRV can compute the \gls{HRV} time series from the beat positions as 
...
has some basic knowledge about both the R environment and \gls{HRV}. 
...
be covered in more depth when they are introduced in the document). The 
descriptions
...
Chapter \ref{ch:installation} explains how to get RHRV installed in 
your computer. This guide assumes that you have already installed R in your 
computer.\\

Chapter \ref{ch:Quick} presents a ``15-minutes guide to RHRV''. This chapter 
presents the essential functions needed to perform basic time and frequency 
domain analysis with RHRV. Chapter \ref{ch:advanced} completes the functionality
introduced in Chapter \ref{ch:Quick} and presents more advanced features 
available in RHRV focusing on reading RR intervals stored in different formats 
and episodic information analysis.\\

Chapter \ref{ch:nonlinear} introduces the functionality needed to perform 
\gls{HRV} nonlinear analysis with RHRV.
