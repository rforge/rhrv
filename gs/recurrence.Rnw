...
To allow experimental implementations, functions from nonlinearTseries are aliased here.
<<>>=
...
%}
@
\todo{propagate parameters from buildTakens and findAllNeighbours in a slot of the result, instead of using explicit parameters in recurrencePlotAux.}
...
%<<unifneighs>>=
%system.time(unifneighs<-local.findAllNeighbours(uniftakens,radius=0.2))[3]
...
\index{Geyser}
\todo{double check: MASS:::geyser should be used, fot faithful}
This is a classical data set with a two dimensional structure, $duration$ and $waiting$.
\todo{Geyser: extend to two-dimensional data}
<<>>=
library(MASS)
data(geyser)
@
...
eruptionstakens4 <- local.buildTakens( time.series=geyser$duration, embedding.dim=4, time.lag=1)
...
eruptionstakens2 <- local.buildTakens(time.series=geyser$duration, embedding.dim=2, time.lag=1)
...
\caption{Recurrence Plot. Example case:  Old geyser Geyser eruptions. Dim=2.
Time used: \Sexpr{laptime()} sec.
}
...
waitingtakens <- local.buildTakens( time.series=geyser$waiting,embedding.dim=4,time.lag=4)
...
gleruptionstakens4 <- local.buildTakens( time.series=geyserlin, embedding.dim=4, time.lag=2)
...
gleruptionstakens2 <- local.buildTakens(time.series=geyserlin, embedding.dim=2, time.lag=2)
...
\caption{Recurrence Plot. Example case:  Old geyserlin Geyser eruptions. Dim=2.
Time used: \Sexpr{laptime()} sec.
}
...
\todo{We have outlies at approximately 2*RR. Could this be an artefact of preprocessing, filtering out too many impulses?}
\begin{figure}[htbp]
\begin{center}
...
\gsnote{We should expect the breathing rythm, so a time lag in the order of 10 is to be expected.}
% dim 2
...
\caption{Recurrence Plot. Example case:  RHRV tutorial. Dim=2, 4, 6, 8, 12, 16.
Time used: \Sexpr{laptime()} sec.
}
...
\caption{Example case: RHRV tutorial. HRRV Signal and linear interpolation.}
...
\caption{Example case: RHRV tutorial. HRRV
Time used: \Sexpr{laptime()} sec.
}
...
\caption{Example case: RHRV tutorial. Ranked HRRV data.
Time used: \Sexpr{laptime()} sec.
}
...
\caption{Recurrence Plot. Example case:  RHRV tutorial. HRRV Dim=4.
Time used: \Sexpr{laptime()} sec.
}
...
% dim 2
\todo{fix default setting for radius. Eckmann uses NN=10}
