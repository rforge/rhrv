...
@
\note{minor cosmetics added to recurrencePlotAux}
<<>>=
...
hrvRRneighs16 <-local.findAllNeighbours(hrvRRtakens16, radius=16)
...
Since we are not interested in heart rate (or pulse), but in heart rate variation, a proposal is to use
<<>>=
...
HRRV <- hrv.data$Beat$dRR/hrv.data$Beat$avRR
@
These are the displays we used before, now for HRV:
%%%%%
...
@
	%<<>>=
	%hrvRRVneighs4 <-local.findAllNeighbours(hrvRRVtakens4, radius=16)
	...
	%hrvRRVneighs2 <-local.findAllNeighbours(hrvRRVtakens2, radius=16)
	...
	%hrvRRVneighs6 <-local.findAllNeighbours(hrvRRVtakens6, radius=16)
	...
	%hrvRRVneighs8 <-local.findAllNeighbours(hrvRRVtakens8, radius=32)
	...
	%hrvRRVneighs16 <-local.findAllNeighbours(hrvRRVtakens16, radius=16)
