...
%global flags for conditional buildss
%:flags
...
\newenvironment{sdanote}{\emph{\textcolor{red}{Note for data analysis:} }}{}
...
%:* task specific
% scale factor - fig name  - source. Source is a graphics with signal/linear
...
%:   insertrecurrence #width    data id    fig.name/label	time}
% set width to 0.9 for split displays
\newcommand\insertrecurrence[4][0.45]{
See \figref{fig:#2}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=#1\linewidth]{recurrence-#2}
\caption{Recurrence plot. #3. Time used: #4 sec.}
\label{fig:#2}
\end{center}
\end{figure}
}
...
% [width] #2: label #3: caption #4: time
\newcommand\insertrqa[4][0.82]{RQA #3. Time used: #4 sec. For graphical ouput, see \figref{fig:recurrence-#2}.
...
Hydrodynamics is a challenging problem. Understanding planetary motion is a historical challenge, and 
may be useful as an illustration.

As a simple illustration, let $x=(x_i)$ be a sequence, maybe near periodic. For now, think of $i$ as a time 
index.

Recurrence plots have two steps. The first was a bold step by Floris Takens. If you do not know the 
phase space of a system, for a choice of ``dimension''  $d$, take the sequence of $d$ tuples taken from 
your data to define the states.
$$ u_i = (x_i,\ldots , x_{i+(d-1)})$$
Under mild conditions, this  Takens' delay embedding allows to reconstruct the topogy of the 
phase space   from this embedded sequence
\citep{takens1981detecting}.
\index{Takens state}
\index{recurrence plot}
\index{delay embedding}

The delay imbedding implicitly depends on the sampling rate with which the signal has been 
sampled. For 
example, if the rate is sufficiently high, all differentiable dynamics appear to be linear sine linear 
approximation holds.  The imbedding construction allows a sub-sampling using a lag parameter $m$. So 
you take
...
the previous information $x_{i-1}$, but for more complex situations you may have to step back in the 
past. 
...
Of course it is possible to compress information here, sorting states and removing duplicates. Keeping 
the 
...
www.recurrence-plot.tk/glance.php>}, for example \figref{fig:RPexamples}

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=1.0\linewidth]{RPexamples}
\caption{Characteristic typology of recurrence plots: (A) homogeneous (uniformly distributed noise), (B) periodic (super-positioned harmonic oscillations), (C) drift (logistic map corrupted with a linearly increasing term) and (D) disrupted (Brownian motion). These examples illustrate how different RPs can be. The used data have the length 400 (A, B, D) and 150 (C), respectively; no embeddings are used; the thresholds are $\epsilon = 0.2$ (A, C, D) and $\epsilon = 0.4$ (B).   From \url{<http://
www.recurrence-plot.tk/glance.php>}}
\label{fig:RPexamples}
\end{center}
\end{figure}
...
analysis (RQA). References: 
\citep{zbilut2006recurrence},  \citep{webber2005recurrence}. See  \Vref{tab:rqa}.
\todo{check RATIO - remove  $RR =^? Radius^2$}

\todo{make scale indpendent}
...
statecoplot(states, main, 
	range  = NULL,
	rank = FALSE, nooverlap = FALSE, 
	 col= rgb(0, 0, 0, rgb(1,0,0, 1/sqrt(dim(states)[1])), 
	number = c(5,5))
\end{verbatim}
\end{Usage}
%
...
\item[\code{rank}] An experimental variant. If \code{rank}, the values are rank transformed.
\item[\code{nooverlap}] An experimental variant. If \code{nooverlap}, the cases are subsampled by dimension.
\item[\code{alpha}] \item[\code{col}]  The current choice is $rgb(1, 0, 0, \alpha = \sqrt{\frac{1}{nr\ states}})$, 
as a default.
...
This assumes that correlated neighbours are collected in lists. Just for reference. This function is inlined.
...
%:   function showrqa()
%RQA Prototype
%<<sin10rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
%sin10rqa <- showrqa(sintakens, radius=0.8, log=TRUE)
%@
%\insertrqa{sin10rqa}{Test case: sinus. Radius=0.8.}{\Sexpr{laptime()}}
<<showrqa>>=
showrqa <- function(takens, dim=NULL, radius, 
		digits=3,		do.hist = TRUE, rm.hist = TRUE, 
		log = TRUE ,...)
{	#takens <- takens[!is.na(takens)]
	...
	cat(id, " n:", dim(takens)[1], " Dim:", dim(takens)[2], " Lag:",xxrqa$time.lag, " Radius:", radius, "\n")
	xxrqa$n <- dim(takens)[1]; 	xxrqa$dim <- dim(takens)[2]

	cat(paste(" Recurrence coverage REC:", round(xxrqa$REC, digits), 
	...
	try(plot(dh, type="h", main=paste( id, " Diagonal:", 
		...
		log = log, ...))	#needs handling of nan, Inf.
	...
	lag<- attr(takens,"time.lag")
	if (!is.null(lag) && (lag !=1) ) pars <- paste(pars, " Lag:", lag)
	plot(drR, type="h", 		main=paste( id," Recurrence Rate",
		pars," Radius: ",radius, " REC:", round(xxrqa$REC, digits)),
...
More test cases used later are the Geyser data set (\Vref{sec:geyser}) and two examples of heart rate data sets (\Vref{sec:hrv} and \Vref{sec:hrv2}) from \code{library(rhrv)}. The Geyser data set gives an example of a bi-variate point process. The HRV data sets give real live point process examples.
...
$10$ sinus waves in  \Sexpr{nsignal} steps.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.52\linewidth]{recurrence-sin10}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.52\linewidth]{recurrence-unif}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.52\linewidth]{recurrence-chirp}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.52\linewidth]{recurrence-doppler}
...
\newcommand\inserttau[3][0.7]{
See \figref{fig:recurrence-#2}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=#1\linewidth]{recurrence-#2}
\caption{$\tau_{delay}$ estimation. #3 Left: based on the autocorrelation function ACF. Right: AMI}
\label{fig:recurrence-#2}
\end{center}
\end{figure}
}
...
@
\subsection{Sinus}
<<acfsin, fig=TRUE, width=10, height=5, include=FALSE>>=
sinN <- sin10()
oldpar <-  par(mfrow=c(1,2))
...
par(oldpar)
cat("Sinus tau.ami:",tau.ami," tau.acf:",tau.acf)
@

 \inserttau{"acfsin"}{"Sinus"}
 
<<embedfsinN, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
...
#taksinN = buildTakens(sinN, embedding.dim = 2, time.lag = 1)
taksinN = buildTakens(sinN, embedding.dim = emb.dim, time.lag = tau.ami)
scatter3D(taksinN[,1], taksinN[,2], taksinN[,3],
...
\subsection{Uniform Random Numbers}
<<acfunif, fig=TRUE, width=10, height=5>>=
unifN <- unif()
oldpar <-  par(mfrow=c(1,2))
# taudelay estimation based on the autocorrelation function
...
par(oldpar)
cat("Uniform ", "tau.acf:", tau.acf ," tau.ami:",tau.ami)
@
<<embedfunifN, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
#<<takfunifN, fig=TRUE>>=
takUnif = buildTakens(unifN, embedding.dim = emb.dim, time.lag = tau.ami)
#range(takUnif[4,])
col4 <- gray((1-takUnif[,4]),0.1)

scatter3D(takUnif[,1], takUnif[,2], takUnif[,3],
	main = paste("Reconstructed phase space","unif n=",length(unifN)),
	col = col4, type="o",cex = 0.3)
par(oldpar)
@
%takUnif = buildTakens(unifN, embedding.dim = emb.dim, time.lag = tau.ami)
%scatter3D(takUnif[,1], takUnif[,2], takUnif[,3],
...
\subsection{Chirp}
<<acfchirp, fig=TRUE, width=10, height=5>>=
chirpN <- chirp()
oldpar <-  par(mfrow=c(1,2))
# taudelay estimation based on the autocorrelation function
...
main=paste("taudelay estimation based on ami\n n=",length(chirpN),"chirp()")) 
par(oldpar)
cat("Chirp ", "tau.acf:", tau.acf ," tau.ami:",tau.ami)
@
<<embedfchirpN, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
#<<takfchirpN, fig=TRUE>>=
takChirp = buildTakens(chirpN, embedding.dim = emb.dim, time.lag = tau.ami)
scatter3D(takChirp[,1], takChirp[,2], takChirp[,3],
	main = paste("Reconstructed phase space\n","Chirp"),
	col = 1, type="o",cex = 0.3)
par(oldpar)
@
%col4 <- gray((takChirp[,4]+1)/2,0.4)
%
%takChirp = buildTakens(chirpN, embedding.dim = emb.dim, time.lag = tau.ami)
%	scatter3D(takChirp[,1], takChirp[,2], takChirp[,3],
%	main = paste("Reconstructed phase space\n","Chirp"),
...
@
\subsection{Doppler}
<<acfdoppler, fig=TRUE, width=10, height=5>>=
dopplerN <- doppler()
oldpar <-  par(mfrow=c(1,2))
# taudelay estimation based on the autocorrelation function
...
main=paste("ami\n",deparse(substitute(x))))
par(oldpar)
cat("Doppler ", "tau.acf:", tau.acf ," tau.ami:",tau.ami)
@

<<embedfdopplerN, fig=TRUE, width=10, height=5 >>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
@
\insertrecurrence{sin10rec}{Test case: sinus}{\Sexpr{laptime()}}


<<sin10rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
sin10rqa <- showrqa(sintakens, radius=0.8, log=TRUE)
@
\insertrqa{sin10rqa}{Test case: sinus. Radius=0.8. log scale}{\Sexpr{laptime()}}
...
\insertrecurrence{sin10lag16rec}{Test case: sinus curve, time lag 16}{\Sexpr{laptime()}}

<<sin10lag16rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
sin10lag16rqa <- showrqa(sintakenslag16, radius=0.2)
@
\insertrqa{sin10lag16rqa}{Test case: sinus curve, time lag 16}{\Sexpr{laptime()}}
...
@
\insertrecurrence{unifrec}{Test case: uniform random numbers}{\Sexpr{laptime()}}

<<uniftakensrqa106, fig=TRUE, include=FALSE , width=10, height=5>>=
showrqa(uniftakens, radius=0.6, log=TRUE)
@
\insertrqa{uniftakensrqa106}{Test case: uniform random numbers, radius=0.6, log scale}{\Sexpr{laptime()}}
...
@
\insertrecurrence{chirprec}{Test case: chirp signal}{\Sexpr{laptime()}}

<<chirprqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(chirptakens, radius=0.6)
@
\insertrqa{chirprqa}{Test case: chirp signal}{\Sexpr{laptime()}}
...
\insertrecurrence{dopplerrec}{Test case: Doppler}{\Sexpr{laptime()}}

<<print=FALSE, echo=FALSE>>=
chunk.time.start<- proc.time()
@

<<dopplerrqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(dopplertakens, radius=0.2)
@
\insertrqa{dopplerrqa}{Test case: Doppler}{\Sexpr{laptime()}}
...
\insertrecurrence{dopplerlag16rec}{Test case: Doppler}{\Sexpr{laptime()}}

<<dopplerlag16rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(dopplertakenslag16, radius=0.2)
@
\insertrqa{dopplerlag16rqa}{Test case: Doppler}{\Sexpr{laptime()}}

\clearpage

\section{Case Study: Geyser example and Bivariate recurrence plots}
\label{sec:geyser}

\subsection{Paired sequences}
\index{Geyser}
...
\begin{sdanote}
The time to predict is the waiting time for the next eruption; so for prediction use a shifted sequence $waiting[-1]$.
\end{sdanote}

\begin{sdanote}
Some data are not recorded, but replaced by codes such as short, long. Handling of partially encoded data is required.
\end{sdanote}
...
Time used: \Sexpr{laptime()} sec.  (Standardized) signal components and linear interpolation. See \figref{fig:recurrence-signals}.
...
\caption{Example case: Old Faithful Geyser eruption standardized  durations and waiting time. Signals and linear interpolation. Time used: \Sexpr{laptime()} sec. }
...
%:duration quickstart
<<acfduration, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
# taudelay estimation based on the autocorrelation function
...
main=paste("taudelay estimation based on ami\n","sin()")) )
par(oldpar)
cat("duration tau.acf:",tau.acf," tau.ami:",tau.ami)
@
 
<<embedfduration, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
<<acfwaiting, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
# taudelay estimation based on the autocorrelation function
...
main=paste("taudelay estimation based on ami\n","waiting()")) )
par(oldpar)
cat("waiting tau.acf:",tau.acf," tau.ami:",tau.ami)
@
 
<<embedfwaiting, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
@
% takens.duration4 replaces  takens.duration4
Time used: \Sexpr{laptime()} sec. 

<<duration4rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
duration4rqa <- showrqa(takens.duration4, radius=0.5, log=TRUE)
@

<<waiting4rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
waiting4rqa <- showrqa(takens.waiting4, radius=0.5, log=TRUE)
@
Time used: \Sexpr{laptime()} sec. For graphical ouput, see \figref{fig:recurrence-waiting4rqa}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.82\linewidth]{recurrence-duration4rqa}

\includegraphics[width=0.82\linewidth]{recurrence-waiting4rqa}
\caption{RQA. Left:Geyser duration. Radius=0.5, log scale. Right:Geyser waiting. Radius=0.5 log scale.}
\label{fig:recurrence-waiting4rqa}
\end{center}
\end{figure}
...
%<<takens.duration4no, fig=TRUE, include=FALSE>>=
%statepairs(takens.duration4, nooverlap=TRUE) #dim=4
%@
%\insertrecurrence{takens.duration4no}{Old Faithful Geyser eruption durations. Dim=4} {\Sexpr{laptime()}}

<<takens_duration4no, fig=TRUE, include=FALSE>>=
	statecoplot(takens.duration4) #dim=4
@

\plotco{takens_duration4no}{Example case: Old Faithful Geyser eruption durations.
Time used: \Sexpr{laptime()} sec.}
...
\caption{Old Faithful Geyser eruption durations and waiting time. Takens states, dim=4. Time used: \Sexpr{laptime()} sec.}
...
\caption{Old Faithful Geyser eruption durations and waiting time. Time used: \Sexpr{laptime()} sec.
}
...
<<recurrencePlot1dw, fig=TRUE, include=FALSE, width=12, height=6>>=
oldpar <- par(mfrow=c(1,2))
...
<<takens_duration4rqa1, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(takens.duration4, radius=0.5)
@
\insertrqa{takens_duration4rqa1}{Example case:  Old Faithful Geyser eruption durations. Radius=2.4 Dim=4}{\Sexpr{laptime()}}

<<takens_waiting4rqa1, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(takens.waiting4, radius=0.5)
@
\insertrqa{takens_waiting4rqa1}{Example case:  Old Faithful Geyser eruption waiting. Radius=0.5 Dim=4}{\Sexpr{laptime()}}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.64\linewidth]{recurrence-recurrencePlot2}
...
So far, the data have been handled as a pair of one-dimensional problems. Looking at the 
data as a bivariate problem requires to use a bivariate distance to define neighbours. 
See  \cite{marwan2002nonlinear}, \cite{marwan2002cross}.

Marwan uses a ``covariance''
$$ CR[i,j] = \Theta( \epsilon - \norm{\vec{x}_i - \vec{y}_j})$$
with a maximum distance  as $\norm{\cdot}$, where $\vec{x}, \vec{y}$ are imbeddings. 
Other distances may be used.
Signed distances for example may be used for experiments.
...
\todo{Epsilon/radius and Heaviside not use here - thresholding is handled in image rendering.}

\todo{propagate names from Takens}
...
\todo{CR0 this needs optimisation on triangle}
<<CR2>>=
...
@
<<cr4, fig=TRUE, include=FALSE, width=10, height=5>>=
oldpar <- par(mfrow=c(1,2))
image(cr4, x=1:ncol(cr4), y=1:nrow(cr4), 
	xlab="duration index", ylab="waiting index",main="CR0 maxdist") 
image(cr4C, x=1:ncol(cr4C), y=1:nrow(cr4C), 
	xlab="duration index", ylab="waiting index", main="CR2 cor distance")
# neighs.matrix <- cr4;range(cr4) #  70.5500 107.1667
par(oldpar)
cat("Range cr4:", range(cr4),"Range cr4C:", range(cr4C, na.rm=TRUE, finite=TRUE))
@
\insertrecurrence[0.8]{cr4}{Cross recurrence. Left:using maxdist distance. Right:cor. Using default colour}{\Sexpr{laptime()}}

A max distance.
<<CRPFromMatrix, fig=TRUE, include=FALSE, width=10, height=5>>=
oldpar <- par(mfrow=c(1,2))
crossrecurrencePlotFromMatrix(cr4, 
	main="Cross Recurrence plot\n max",
	xlab="duration4 index", ylab="waiting4 index")  # ugly heat matrix
#@
#\insertrecurrence{CRPFromMatrix}{Cross recurrence ??using max distance}{\Sexpr{laptime()}}
#
...
par(oldpar)
@
%\insertrecurrence{crossrecurrencePlot}{Cross recurrence ??using max distance}{\Sexpr{laptime()}}
\insertrecurrence[0.8]{CRPFromMatrix}{Cross recurrence using max distance - default colours and grey scale}{\Sexpr{laptime()}}

<<corrdist, fig=TRUE, include=FALSE, width=10, height=5>>=
oldpar <- par(mfrow=c(1,2))
...
par(oldpar)
@
\insertrecurrence[0.8]{corrdist}{Cross recurrence using cor distance - default colours and grey scale}{\Sexpr{laptime()}}

\clearpage
%: start hidden geyser ******************
%    \section{Case Study: Geyser data --defunct}
%        \label{sec:geyserdefunct}
%        \index{Geyser}
%        
...
%    @
%    %
%    %takens.duration4 <- takens.duration4
%    
%    %:**** here
%    %    <<takens.duration4no1, fig=TRUE, include=FALSE>>=
%    %    	statepairs(takens.duration4no1, nooverlap=TRUE) #dim=4
%    %    @
%    %    \begin{figure}[htbp]
%    %    \begin{center}
%    %    \includegraphics[width=1.0\linewidth]{recurrence-takens.duration4no1}
%    %    \caption{Example case: Old Faithful Geyser eruption durations.
%    %    Time used: \Sexpr{laptime()} sec.
%    %     }
%    %    
%    %    \label{fig:takens.duration4no1}
%    %    \end{center}
%    %    \end{figure}
%    %    
%    %:03.06.2017 to delete
...
%    @
%    \insertrecurrence{eruptionstakens2}{Old Faithful Geyser eruption durations. Dim=2} {\Sexpr{laptime()}}%
%    %
...
%    @
%    
%    \insertrecurrence{eruptionsrec2}{Old Faithful Geyser eruption durations. Dim=2} {\Sexpr{laptime()}}
%    
%    <<eruptionstakens2rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
%    showrqa(eruptionstakens2, radius=0.8)
%    @
%    \insertrqa{eruptionstakens2rqa}{Example case:  Old Faithful Geyser eruption durations. Dim=2}{\Sexpr{laptime()}}
%    
...
%    @
%    \insertrecurrence{eruptionsrec42}{Example case:  Old Faithful Geyser eruption durations. Dim=4} {\Sexpr{laptime()}}
%    
%    <<takens_duration4rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
%    showrqa(takens.duration4, radius=0.8)
%    @
%    \insertrecurrence{takens_duration4rqa}{Old Faithful Geyser eruption durations. Dim=4}{\Sexpr{laptime()}}
%    
...
%    @
%    \insertrecurrence{eruptionstakens8}{Old Faithful Geyser eruption durations. Dim=8}{\Sexpr{laptime()}}
%    %
%    %
%    @
...
%    @
%    \insertrecurrence{eruptionsrec8}{Example case:  Old Faithful Geyser eruption durations. Dim=8} {\Sexpr{laptime()}}
%    
%    <<eruptionstakens8rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
%    showrqa(eruptionstakens8, radius=2.6)
%    @
%    \insertrqa{eruptionstakens8rqa}{Example case:  Old Faithful Geyser eruption durations. Dim=8}{\Sexpr{laptime()}}
%    
%    \clearpage
...
%    chunk.time.start<- proc.time()
%    @
%    <<waitingneighsrqa, fig=TRUE, include=FALSE, width=10, height=5>>=
%    showrqa(waitingtakens, radius=16)
%    @
%    \insertrqa{waitingneighsrqa}{Example case:  Old Faithful Geyser waiting}{\Sexpr{laptime()}}
%    
%    <<waitingrec, fig=TRUE, include=FALSE>>=
%    load(file="waitingneighs.RData")
%    local.recurrencePlotAux(waitingneighs)
%    @
%    \insertrecurrence{waitingrec}{Example case:  Old Faithful Geyser waiting}{\Sexpr{laptime()}}
%    
...
%    %chunk.time.start<- proc.time()
%    %@
%    %<<glintakens.duration4, fig=TRUE, include=FALSE>>=
%    %glintakens.duration4 <- 
%    %	local.buildTakens( time.series=geyserlin, 
%    %		embedding.dim=4, time.lag=2)
%    %statepairs(glintakens.duration4) #dim=4
%    %@
%    %See \figref{fig:glintakens.duration4}.
%    %@
%    %\begin{figure}[htbp]
%    %\begin{center}
%    %\includegraphics[width=0.8\linewidth]{recurrence-glintakens.duration4}
%    %\caption{Example case: Old Faithful Geyser eruptions.
%    %Time used: \Sexpr{laptime()} sec.
%    % }
%    %\label{fig:glintakens.duration4}
%    %\end{center}
%    %\end{figure}
%    %%<<glineruptionsneighs>>=
%    %%
%    %
%    %<<gl01>>=
%    %glineruptionsneighs4 <- local.findAllNeighbours(glintakens.duration4, radius=0.8)
%    %save(glineruptionsneighs4, file="glineruptionsneighs4.RData")
%    %<<print=FALSE, echo=FALSE>>=
%    %chunk.time.start<- proc.time()
%    %@
%    %<<glintakens.duration4rqa, fig=TRUE, include=FALSE>>=
%    %showrqa(glintakens.duration4, radius=0.8)
%    %@
%    %\insertrecurrence{glintakens.duration4rqa}{Old Faithful Geyser - both. Dim=4.
%    %Time used: \Sexpr{laptime()} sec.
%    %}
%    %
%    %
...
\section{Case Study: Tension data}
\label{sec:tension}
...
\sdanote{a logistic transform is used in \ref{subsec:tension1}.}

The tension data set (from \citep{ebner2007state}, \citep{Ebner-Priemer2007Ambulatory-asse}) is one 
sample case of recordings of psychological tension data, recorded in regular interval on a subjective 
tension level ranked $0 \ldots 10$. The data are ranked data, and may contain missing data. Missing data  
are just removed here.

<<tension, fig=TRUE, width=10, height=5>>=
tension <- scan("../media/tension.dat")
plotsignal(tension,"tension","level")
tension<-tension[!is.na(tension)]
@
\todo{fix segfault in \code{mutualInformation()} if NA is included}
% this crashes, e.g.
%*** caught segfault ***
%address 0x7fd5e8603618, cause 'memory not mapped'
%
%Traceback:
% 1: mutualInformation(time.series, lag.max = lag.max, do.plot = do.plot,     main = main, ...)
% 2: timeLag(tension, technique = "ami", do.plot = T, selection.method = "first.minimum",     main = paste("$\tau_{delay}$ estimation based on ami\n",         "tension()"))
%
%Possible actions:
...
<<acftension, fig=TRUE, width=10, height=5, include=FALSE>>=
...
try(tau.ami <- timeLag(tension, technique = "ami", 
	...
	main=paste("$\tau_{delay}$ estimation based on ami\n","tension()")) )
par(oldpar)
cat("tension tau.acf:",tau.acf," tau.ami:",tau.ami)
@
 \inserttau{"acftension"}{"Tension"}
 
<<embedtension, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
#%<<takfsinN, fig=TRUE>>=
# taktension = buildTakens(tension, embedding.dim = 6, time.lag = 1)
...
par(oldpar)
@
<<tensionrec, fig=TRUE, include=FALSE>>=
tensionneighs <- local.findAllNeighbours(taktension, radius=2.0)
save(tensionneighs, file="tensionneighs.Rdata")
 # load(file="tensionneighs.Rdata")
local.recurrencePlotAux(tensionneighs, dim=dim(taktension)[2], radius=2.0)
@
\insertrecurrence{tensionrec}{Test case: tension}{\Sexpr{laptime()}}


<<tensionrqa, fig=TRUE, include=FALSE, width=10, height=5>>=
tensionrqa <- showrqa(taktension, radius=2.0, log=TRUE)
@
\insertrqa{tensionrqa}{Test case: tension. Radius=2.0}{\Sexpr{laptime()}}
...
%> lp <- log(p/(1-p))
%> lp
% [1] -2.3025851 -1.5040774 -0.9808293 -0.5596158 -0.1823216  0.1823216  0.5596158
% [8]  0.9808293  1.5040774  2.3025851
%> plot(lp)
@

\subsection{Tension: transformed scale}
\label{subsec:tension1}
Try rescaled logits:
...
<<acftension1, fig=TRUE, width=10, height=5, include=FALSE>>=
...
try(tau.ami <- timeLag(tension1, technique = "ami", 
	...
	main=paste("$\tau_{delay}$ estimation based on ami\n","tension1()")) )
par(oldpar)
cat("tension rescaled. tau.acf:",tau.acf," tau.ami:",tau.ami)
@
 \inserttau{"acftension1"}{"tension1"}
 
<<embedtension1, fig=TRUE, width=10, height=5>>=
oldpar <-  par(mfrow=c(1,2))
if (is.numeric(tau.ami)) {
...
par(oldpar)
@
<<tension1rec, fig=TRUE, include=FALSE>>=
tension1neighs <- local.findAllNeighbours(taktension1, radius=1.4)
save(tension1neighs, file="tension1neighs.Rdata")
# load(file="tension1neighs.Rdata")
local.recurrencePlotAux(tension1neighs, dim=dim(taktension1)[2], radius=1.4)
@
\insertrecurrence{tension1rec}{Test case: tension1}{\Sexpr{laptime()}}


<<tension1rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
tension1rqa <- showrqa(taktension1, radius=2, log=TRUE)
@
\insertrqa{tension1rqa}{Test case: tension1. Radius=1.4. log scale}{\Sexpr{laptime()}}
...
\begin{sdanote}
The data include missing beats, or spurious artefacts. These need to be handled. Graphical display is heavily affected by the scales implied by outliers.
\end{sdanote}
...
@
\plotpairs{hrvRRtakens4sp}{RHRV tutorial example.beats.}{\Sexpr{laptime()}}

<<hrvRRtakens4rk, fig=TRUE, include=FALSE>>=
statepairs(hrvRRtakens4, rank=TRUE) #dim=4
@
\plotpairs{hrvRRtakens4rk}{RHRV tutorial example.beats.  Ranked data}{\Sexpr{laptime()}}
...
\insertrecurrence{hrvRRtakens4}{RHRV tutorial example.beats}{\Sexpr{laptime()}}
<<>>=
...
@
\insertrecurrence{hrvRRrec4}{Example: RHRV tutorial example.beats. Dim=4.}{\Sexpr{laptime()}}
...
showrqa(hrvRRtakens2, do.hist=TRUE, radius=16)
@
%\insertrecurrence{hrvRRrec2}{Dim=2.}{\Sexpr{laptime()}}
...
@
\plotpairs{hrvRRVtakens4}{RHRV tutorial example.beats. HRRV}{\Sexpr{laptime()}}

<<hrvRRVtakens4rk, fig=TRUE, include=FALSE>>=
statepairs(hrvRRVtakens4, rank=TRUE) #dim=4
@
\plotpairs{hrvRRVtakens4rk}{RHRV tutorial example.beats ranked  HRRV data}{\Sexpr{laptime()}}
...
@
\insertrecurrence{hrvRRVrec4}{RHRV tutorial example.beats. HRRV Dim=4}{\Sexpr{laptime()}}
...
@
Dim=2. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomp1}
<<hrvRRVrec2rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens2[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrvRRVrec2}{HRRV Dim=2}{\Sexpr{laptime()}}
...
@
Dim=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomp1}
<<hrvRRVrec6rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens6[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrvRRVrec6rqa}{HRRV Dim=6}{\Sexpr{laptime()}}
...
@
Dim=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomp1}
<<hrvRRVrec8rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens8[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrvRRVrec8rqa}{HRRV Dim=8}{\Sexpr{laptime()}}
...
@
Dim=12. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomp1}
<<hrvRRVrec12rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens2[-(1:2),], radius=3/16, do.hist=TRUE)
@
\insertrqa{hrvRRVrec12rqa}{HRRV Dim=12}{\Sexpr{laptime()}}
...
Dim=12. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomp1}
<<hrvRRVrec12rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens12[-(1:2),], radius=3/16, do.hist=TRUE)
@
\insertrqa{hrvRRVrec12rqa}{HRRV Dim=12}{\Sexpr{laptime()}}
...
\label{fig:recurrence-hrvRRVreccomp1}
\end{center}
\end{figure}
%:
\clearpage
...
@
Dim=2, lag=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomplag8}.
<<hrvRRVrec2Lag08rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens2Lag08[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrvRRVrec2Lag08rqa}{HRRV Dim=2, lag=8}{\Sexpr{laptime()}}
...
@
Dim=4, lag=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomplag8}.
<<hrvRRVrec4Lag08rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens4Lag08[-(1:2),], radius=0.125, do.hist=TRUE)
@
...
@
Dim=6, lag=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomplag8}.
<<hrvRRVrec6Lag08rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens6Lag08[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrvRRVrec6Lag08rqa}{HRRV Dim=6, lag=8}{\Sexpr{laptime()}}
...
@
Dim=8, lag=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomplag8}.
<<hrvRRVrec8Lag08rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens8Lag08[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrvRRVrec6Lag08rqa}{HRRV Dim=8, lag=8}{\Sexpr{laptime()}}
@
...
@
Dim=12, lag=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomplag8}.
<<hrvRRVrec12Lag08rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens12Lag08[-(1:2),], radius=3/16, do.hist=TRUE)
@
\insertrqa{hrvRRVrec12Lag08rqa}{HRRV Dim=12, lag=8}{\Sexpr{laptime()}}
@
...
@
Dim=16, lag=8. Time used: \Sexpr{laptime()} sec. See \ref{fig:recurrence-hrvRRVreccomplag8}.
<<hrvRRVrec16Lag08rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
showrqa(hrvRRVtakens16Lag08[-(1:2),], radius=3/16, do.hist=TRUE)
@
\insertrqa{hrvRRVrec16Lag08rqa}{HRRV Dim=16, lag=8}{\Sexpr{laptime()}}
@
...
\label{fig:recurrence-hrvRRVreccomplag8}
\end{center}
\end{figure}
%:
\clearpage
...
@
\plotpairs{hrv2RRtakens4rk}{RHRV tutorial example2.beats. Ranked data}{\Sexpr{laptime()}
...
<<hrv2RRrec4, fig=TRUE, include=FALSE, width=10, height=5>>=
...
showrqa(hrv2RRtakens4[-(1:2),], radius=12*16, do.hist=TRUE)
@
...
<<hrv2RRrec12rqa, fig=TRUE, include=FALSE>>=
hrv2RRrqa2 <- showrqa(hrv2RRtakens2[-(1:2),], radius=10*16, do.hist=TRUE)
@
\insertrqa{hrv2RRrec12rqa}{hrv2RR dim=2}{\Sexpr{laptime()}}
...
<<hrv2RRrec6, fig=TRUE, include=FALSE, width=10, height=5>>=
...
showrqa(hrv2RRtakens6[-(1:2),], radius=14*16, do.hist=TRUE)
@
Dim=6. Time used: \Sexpr{laptime()} sec.
...
<<hrv2RRrec8, fig=TRUE, include=FALSE, width=10, height=5>>=
...
showrqa(hrv2RRtakens8[-(1:2),], radius=16*16, do.hist=TRUE)
@
Dim=8. Time used: \Sexpr{laptime()} sec.
...
<<hrv2RRrec12, fig=TRUE, include=FALSE, width=10, height=5>>=
...
showrqa(hrv2RRtakens12[-(1:2),], radius=16*16, do.hist=TRUE)
@
Dim=12. Time used: \Sexpr{laptime()} sec.
...
<<hrv2RRrec16, fig=TRUE, include=FALSE, width=10, height=5>>=
...
showrqa(hrv2RRtakens16[-(1:2),], radius=18*16, do.hist=TRUE)
@
Dim=16. Time used: \Sexpr{laptime()} sec.
...
@
\insertrecurrence{hrv2RRVtakens4}{RHRV tutorial example2.beats. HRRV}{\Sexpr{laptime()}}
%
...
@
\insertrecurrence{hrv2RRVtakens4l}{RHRV tutorial example2.beats. HRRV}{\Sexpr{laptime()}}
%
...
@
\todo{check. There seem to be strange artefacts.}
\insertrecurrence{hrv2RRVrec4}{RHRV tutorial example2.beats hrv2RRV. Dim=4}{\Sexpr{laptime()}}
...
@
See \ref{fig:recurrence-hrv2RRVreccomp}.
<<hrv2RRVrec2rqa, fig=TRUE, include=FALSE>>=
hrv2RRVrqa2 <- showrqa(hrv2RRVtakens2[-(1:2),], radius=0.125, do.hist=TRUE)
@
\insertrqa{hrv2RRVrec2rqa}{hrv2RR dim=2}{\Sexpr{laptime()}}
...
@
See \ref{fig:recurrence-hrv2RRVreccomp}.
<<hrv2RRVrec6rqa, fig=TRUE, include=FALSE>>=
hrv2RRVrqa6 <- showrqa(hrv2RRVtakens6[-(1:2),], radius=0.125*1.5, do.hist=TRUE)
@
\insertrqa{hrv2RRVrec6rqa}{hrv2RR dim=6}{\Sexpr{laptime()}}
...
@
See \ref{fig:recurrence-hrv2RRVreccomp}.
<<hrv2RRVrec8rqa, fig=TRUE, include=FALSE>>=
hrv2RRVrqa8 <- showrqa(hrv2RRVtakens8[-(1:2),], radius=3/16, do.hist=TRUE)
@
\insertrqa{hrv2RRVrec8rqa}{hrv2RR dim=8}{\Sexpr{laptime()}}
...
@
See \ref{fig:recurrence-hrv2RRVreccomp}.
<<hrv2RRVrec12rqa, fig=TRUE, include=FALSE>>=
hrv2RRVrqa12 <- showrqa(hrv2RRVtakens12[-(1:2),], radius=3.5/16, do.hist=TRUE)
@
\insertrqa{hrv2RRVrec12rqa}{hrv2RR dim=12}{\Sexpr{laptime()}}
...
@
See \ref{fig:recurrence-hrv2RRVreccomp}.
<<hrv2RRVrec16rqa, fig=TRUE, include=FALSE>>=
hrv2RRVrqa16 <- showrqa(hrv2RRVtakens16[-(1:2),], radius=4/16, do.hist=TRUE)
@
\insertrqa{hrv2RRVrec16rqa}{hrv2RR dim=16}{\Sexpr{laptime()}}
...
\bibliography{../pulse,../sda}% no space!!
%
\clearpage
