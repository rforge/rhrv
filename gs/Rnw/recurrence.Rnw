...
% R CMD Rdconv -t latex statepairs.Rd -o statepairs.tex
\HeaderA{statepairs}{}{statepairs}
...
\begin{Arguments}
\begin{ldescription}
\item[\code{states}] A matrix: states by dimension.
...
par(mfrow=c(1,2))
plot(signal, 
	main="",col=rgb(0,0,1,0.3), pch=20, xlab="index" )

plot(signal, type="l", 
	main="", xlab="index", col=rgb(0,0,0,0.4))
points(signal, col=rgb(0,0,1,0.3), pch=20 )
...
eruptionstakens6 <- local.buildTakens( time.series=geyser$duration,
...
@
<<>>=
eruptionsneighs6<-local.findAllNeighbours(eruptionstakens6, 
	radius=0.8)
...
gleruptionstakens6 <- local.buildTakens( time.series=geyserlin,
...
gleruptionstakens8 <- local.buildTakens( time.series=geyserlin,
...
@
<<gl04>>=
eruptionsneighs8<-local.findAllNeighbours(gleruptionstakens8, 
	radius=0.8)
save(eruptionsneighs8, file="eruptionsneighs8.RData")
<<glprint=FALSE, echo=FALSE>>=
chunk.time.start<- proc.time()
