...
@
\insertrqua{takens_duration4rqa1}{Example case:  Old Faithful Geyser eruption durations. Dim=4}{\Sexpr{laptime()}}
...
So far, the data have been handled as a pair of one-dimensional problems. Looking at the data as a bivariate problem requires to use a bivariate distance to define neighbours. See \citep{marwan2002nonlinear}.
