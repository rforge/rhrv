...
\gsnote{this is an early draft}

Marwan uses a ``covariance''
$$ CR[i,j] = \Theta( \epsilon - \norm{xv[i] - yv[j]})$$
a maximum distance  as $\norm{\cdot}$
...
@
<<cr4, fig=TRUE, include=FALSE>>=
image(cr4)
# neighs.matrix <- cr4;range(cr4) #  70.5500 107.1667
@
\insertrecurrence{cr4}{cr4 - Cross recurrence ??using cor distance}{\Sexpr{laptime()}}

A max distance.
<<CRPFromMatrix, fig=TRUE, include=FALSE>>=
...
<<corrdist, fig=TRUE, include=FALSE>>=
...
# introducing radius/cut by zlim
@
\insertrecurrence{CRPFromMatrixC}{Cross recurrence ??using cor distance}{\Sexpr{laptime()}}
