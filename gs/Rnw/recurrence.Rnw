...
\newcommand\insertrecurrence[4][0.64]{
See \figref{fig:#2}.
...
\begin{center}
\includegraphics[width=#1\linewidth]{recurrence-#2}
\caption{$\tau_{delay}$ estimation. #3 Left: based on the autocorrelation function. Right ACF: AMI}
\label{fig:recurrence-#2}
\end{center}
\end{figure}
}
...
@
\insertrqua{dopplerlag16rqa}{RQA. Test case: Doppler}{\Sexpr{laptime()}}

\clearpage

\section{Geyser example and Bivariate recurrence plots}
\label{sec:geyser}
...
duration4rqa <- showrqa(takens.duration4, radius=0.8, log=TRUE)
@
\insertrqua{duration4rqa}{Geyser duration. Radius=0.8.}{\Sexpr{laptime()}}

<<waiting4rqa, fig=TRUE, include=FALSE, width=10, height=5>>=
waiting4rqa <- showrqa(takens.waiting4, radius=2.4, log=TRUE)
@
\insertrqua{waiting4rqa}{Geyser waiting. Radius=2.4.}{\Sexpr{laptime()}}
...
<<>>=
durationneighs4<- findAllNeighbours(takens.duration4, radius=2.4)
...
showrqa(takens.duration4, radius=3*0.8)
@
...
a maximumdistance  as $\norm{}$

Here is a brute force implmentaation
...
@
\todo{Epsilon/radius and heaviside not use here - may be handled in image rendering.}
...
@
Variant $CR2$ alows a distance that is not facored via $x-y$.
<<CR2>>=
...
\insertrecurrence{duration_takens4L1}{Old Faithful Geyser eruption durations}{\Sexpr{laptime()}}
