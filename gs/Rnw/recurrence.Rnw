...
%
%% Defunct. Master is in RHRV now. This is copied to RHRV. The master file is in lectures/src/dataanalysis.
%  Adjust the path names below and run the R command to process this file
%
% oldoptions<-options(width=72,prompt="  ", continue="  "); options(oldoptions)
...
\subsubsection{Takens' Recurrence States}
\index{Takens state}
...
Takens states are vectors in $M$ dimensions. There are standard statistical techniques to visiulaize aspects of $M$  dimensional vectors, at least for not too high dimension. One is the draftsman's plot, a scatterplot matrix by marginals. For Takens states, this is implemented as \code{statepairs()}. The other is coplots, a variant of scatterplot matrix by marginals, contitionend by one or two additional variables. For Takens states, this is implemented as \code{statecolplots()}
...
Assuming the index is time, the \code{stateplot()} is layed out to show the highest index as response.
...
\todo{improve to a full \code{show} method}
%:   function showrqa
<<showrqa>>=
showrqa <- function(takens, dim=NULL, radius, do.hist = TRUE)
{
	...
	cat(paste("Radius:", radius, 
		" Recurrence coverage REC:", round(xxrqa[1]$REC, 3), 
		" log(REC)/log(R):", round(log(xxrqa[1]$REC)/log(radius),3), "\n"))
	cat(paste("Determinism:", round(xxrqa$DET,3), 
		" Laminarity:",round(xxrqa$LAM,3), "\n"))
	cat(paste("DIV:", round(xxrqa$DIV,3), "\n"))
	cat(paste("Trend:", round(xxrqa$TREND,3), 
		" Entropy:",round(xxrqa$ENTR,3), "\n"))
	cat(paste("Diagonal lines max:", round(xxrqa$Lmax,3),
		 " Mean:",round(xxrqa$Lmean,3), 
		" Mean off main:",round(xxrqa$LmeanWithoutMain,3), "\n"))
	cat(paste("Vertical lines max:", round(xxrqa$Vmax,3), 
		" Mean:",round(xxrqa$Vmean,3), "\n"))
		# str(xxrqa[4:12])
...
More test cases used later are the Geyser data set (\vref{sec:geyser}) and two examples of heart rate data sets (\vref{sec:hrv} and \vref{sec:hrv2}).
...
\plotrecurrence{sin10rec}{Recurrence Plot. Test case: sinus curves.
Time used: \Sexpr{laptime()} sec.}
...
@
\plotrecurrence{sin10rqa}{RQA. Test case: sinus curves.
Time used: \Sexpr{laptime()} sec.
}
...
@
\plotrecurrence{unifrec}{Recurrence Plot. Test case: uniform random numbers.
Time used: \Sexpr{laptime()} sec.
}
...
@
\plotrecurrence{chirprec}{Recurrence Plot. Test case: chirp signal.
Time used: \Sexpr{laptime()} sec.
}
...
\plotrecurrence{eruptionstakens4}{Example case: Old Faithful Geyser eruptions.
Time used: \Sexpr{laptime()} sec.
 }
%
...
\plotco{eruptionstakens4co}{Example case: Old Faithful Geyser eruptions.
Time used: \Sexpr{laptime()} sec.
 }
%
...
@
\plotrecurrence{eruptionstakens4no}{Example case: Old Faithful Geyser eruptions.
Time used: \Sexpr{laptime()} sec.
 }
%
...
@
\plotrecurrence{eruptionsrec4}{Recurrence Plot. Example case:  Old Faithful Geyser eruptions. Dim=4.
Time used: \Sexpr{laptime()} sec.
}
...
@
\plotrecurrence{eruptionstakens2}{Example case: Old Faithful Geyser eruptions. Dim=2.
Time used: \Sexpr{laptime()} sec.
 }
%
%
...
\plotrecurrence{eruptionsrec2}{Recurrence Plot. Example case:  Old Faithful Geyser Geyser eruptions. Dim=2.
Time used: \Sexpr{laptime()} sec.
}
...
statepairs(eruptionstakens4) #dim=4
@
\plotrecurrence{eruptionstakens4}{Example case: Old Faithful Geyser eruptions. Dim=4.
Time used: \Sexpr{laptime()} sec.
 }
%
%
...
statepairs(eruptionstakens4, nooverlap=TRUE) #dim=4
@
\plotrecurrence{eruptionstakens4no}{Example case: Old Faithful Geyser eruptions. Dim=4.
Time used: \Sexpr{laptime()} sec.
 }
%
...
@
\plotrecurrence{eruptionstakens8}{Example case: Old Faithful Geyser eruptions. Dim=8.
Time used: \Sexpr{laptime()} sec.}
%
%
@
...
@
\plotrecurrence{eruptionsrec8}{Recurrence Plot. Example case:  Old Faithful Geyser eruptions. Dim=8.
Time used: \Sexpr{laptime()} sec.
}
...
See  \figref{fig:glineruptionstakens4rqa}.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.5\linewidth]{recurrence-glineruptionstakens4rqa}
\caption{Recurrence Plot. Example case:  Old Faithful Geyser eruptions. Dim=4.
Time used: \Sexpr{laptime()} sec.
}
\label{fig:glineruptionstakens4rqa}
\end{center}
\end{figure}
...
@
\plotrecurrence{glineruptionsrec4}{Recurrence Plot. Example case:  Old Faithful Geyser eruptions. Dim=4.
Time used: \Sexpr{laptime()} sec.}
...
@
\plotrecurrence{glineruptionsrec2}{Recurrence Plot. Example case:  Old Faithful Geyser eruptions linearized. Dim=2.
Time used: \Sexpr{laptime()} sec.
}
...
\subsection{RHRV:  example.beats - Variation: Comparison by Dimension}
...
\subsection{RHRV:  example2.beats - Comparison by Dimension}
...
<<hrv2RRtakens2, fig=FALSE, include=FALSE>>=
...
<<hrv2RRtakens6, fig=TRUE, include=FALSE>>=
...
@
% dim 8
<<hrv2RRtakens8, fig=TRUE, include=FALSE>>=
...
<<hrv2RRtakens12, fig=TRUE, include=FALSE>>=
...
<<hrv2RRtakens16, fig=TRUE, include=FALSE>>=
...
\subsection{RHRV:  example2.beats - Variation: Comparison by Dimension}
...
<<hrv2RRVtakens6, fig=TRUE, include=FALSE>>=
...
@
% dim 8
<<hrv2RRVtakens8, fig=TRUE, include=FALSE>>=
...
<<hrv2RRVtakens12, fig=TRUE, include=FALSE>>=
hrv2RRVtakens12 <- 
	local.buildTakens( time.series=HRRV[1:nsignal],
		embedding.dim=12,time.lag=1)
hrv2RRVneighs12 <- 
...
<<hrv2RRVtakens16, fig=TRUE, include=FALSE>>=
