...
\newcommand\insertrqua[4][0.64]{RQA #3. Time used: #4 sec. For graphical ouput, see \figref{fig:recurrence-#2}.
...
{	#takens <- takens[!is.na(takens)]
	...
	if (do.hist){
	if (log==TRUE) log<-"y"; 	oldpar <- par(mfrow=c(2,1))
...
\begin{center}
\includegraphics[width=#1\linewidth]{recurrence-#2}
\caption{\tau_{delay} estimation. #3 Left: based on the autocorrelation function. Right: AMI}
\label{fig:recurrence-#2}
\end{center}
\end{figure}
}
...
@
\subsection{Sinus}
<<acfsin, fig=TRUE, width=10, height=5>>=
sinN <- sin10()
...
try(tau.ami <- timeLag(sinN, technique = "ami", do.plot = T, selection.method="first.minimum",
main=paste("taudelay estimation based on ami\n","sin()")) )
...
<<duration4rqa, fig=TRUE, include=FALSE>>=
...
<<waiting4rqa, fig=TRUE, include=FALSE>>=
...
<<takens_duration4rqa1, fig=TRUE, include=FALSE>>=
...
<<takens_duration4rqa, fig=TRUE, include=FALSE>>=
