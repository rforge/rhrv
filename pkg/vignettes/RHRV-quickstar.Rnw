...
% \VignetteDepends{RHRV}
% \VignetteEngine{highlight::highlight}
% \VignetteDepends{Rcpp, highlight}
% \VignetteKeyword{HRV}
...
\begin{document}
\SweaveOpts{concordance=TRUE, prefix.string = figures/quickstart, highlight=TRUE} 
...
reader is referred to the free tutorial \cite{rhrv-tutorial} for further 
information. 
}
...
in the online tutorial \cite{rhrv-tutorial}.
...
\textit{STFT}. Thus, it is highly recommended to select \textit{sizesp} so that 
...
\textit{MODWPT} tree \cite{percival2006}. The relative error $\epsilon_r$ of $f_l$ 
...
frequency bands (usually both \textit{ULF} and \textit{VLF} bands).\\
