...
#    "all" includes all types
   ...
   if (!is.null(Tag)) {
	...
	# --------------------
		...
		write(seq(from=0,to=lframes,length.out=length(HRVData$FreqAnalysis[[indexFreqAnalysis]]$HRV)),file=xvector,ncolumns=1)
		lfhfvector=tempfile(pattern = "RHRV", tmpdir = tempdir())
		write(HRVData$FreqAnalysis[[indexFreqAnalysis]]$LFHF,file=lfhfvector,ncolumns=1)

		ulfvector=tempfile(pattern = "RHRV", tmpdir = tempdir())
		write(HRVData$FreqAnalysis[[indexFreqAnalysis]]$ULF,file=ulfvector,ncolumns=1)

		vlfvector=tempfile(pattern = "RHRV", tmpdir = tempdir())
		write(HRVData$FreqAnalysis[[indexFreqAnalysis]]$VLF,file=vlfvector,ncolumns=1)

		lfvector=tempfile(pattern = "RHRV", tmpdir = tempdir())
		write(HRVData$FreqAnalysis[[indexFreqAnalysis]]$LF,file=lfvector,ncolumns=1)

		hfvector=tempfile(pattern = "RHRV", tmpdir = tempdir())
		write(HRVData$FreqAnalysis[[indexFreqAnalysis]]$HF,file=hfvector,ncolumns=1)

		pythonscript=paste(pythonscript,xvector,lfhfvector,ulfvector,vlfvector,lfvector,hfvector)

		if (numfilas==6) {
		   xvector2=tempfile(pattern = "RHRV", tmpdir = tempdir())
			lsecs=tail(HRVData$Beat$Time,1)-head(HRVData$Beat$Time,1)
			write(seq(from=0,to=lsecs,length.out=length(HRVData$HR)),file=xvector2,ncolumns=1)

		   hrvector=tempfile(pattern = "RHRV", tmpdir = tempdir())
			write(HRVData$HR,file=hrvector,ncolumns=1)
			pythonscript=paste(pythonscript,xvector2,hrvector)
		}

		if ((normalized == TRUE) && (numfilas == 6)) 
			stringtitle="Normalized_power_bands_of_HRV_and_Heart_Rate_Signal"
		else if ((normalized == FALSE) && (numfilas == 6))
			stringtitle="Power_bands_of_HRV_and_Heart_Rate_Signal"
		else if ((normalized == TRUE) && (numfilas != 6))
			stringtitle="Normalized_power_bands_of_HRV"
		else
